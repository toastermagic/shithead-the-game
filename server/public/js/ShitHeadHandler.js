class ShitHeadHandler extends GameScene {
    ...
    canPlay(card, player) { // <-- only called if the local player relocates a card
        ...
        var cards = throwStack.containingCards.filter((c) => c.cardValue !== 3); // <-- remove all sevens because they are transparent
        ...
        } else if (cardValue === 3 || card.cardType === JOKER) // these cards can be thrown all the time
        {
            console.log("mayCardBeThrown()", "3 or JOKER");
            return true;
        ...
        } else if (cardValue === 2) // these cards can be thrown on all but the joker
        {
            ...
            console.log("mayCardBeThrown()", "10s on anything but 9");
            return underlayingValue !== 9;
        }
